...
## 4-1. ASR(음성 전사) 결과의 성질

자막이 없어 직접 전사한 것은 **자동 자막과 같은 등급**이다. 다른 점과 같은 점을 구분한다.

- **화자를 구분하지 않는다.** 자동 자막과 같다. 귀속은 여전히 추정이다.
- **모델 크기가 고유명사를 가른다.** 같은 한국어 강연 첫 30초에서 base는 "못돼 이노베이트"·"로드의 존버통신",
  small은 "롯데 이노베이트"·"로떼 종보통신"으로 옮겼다. 조직명을 다루는 자료에서 base는 쓸 수 없다.
  small도 "롯데정보통신"을 "종보통신"으로 흘린다 — **조직명은 영상 제목·설명으로 따로 확인한다.**
- **시각은 정확하다.** 구간 시각은 오디오 위치에서 직접 나오므로 인용 위치로 쓸 수 있다.
- **모델과 백엔드를 기록한다.** `transcript_source`에 "asr (whispercpp small)"처럼 남긴다.
  나중에 같은 파일을 더 큰 모델로 다시 돌릴지 판단하는 근거가 된다.
- **비용을 계산하고 시작한다.** CPU에서 small은 실시간의 3.4배다. 한 시간짜리가 18분이다.
  100편을 돌리기 전에 대상을 골라야 한다.
