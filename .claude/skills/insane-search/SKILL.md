...
<!-- vendored copy: see UPSTREAM.md. The upstream first-run setup (GitHub-star prompt, update-notifier hook) is removed; nothing here writes to ~/.claude or asks questions. -->
> **Vendored in this repository.** Upstream: fivetaku/insane-search 0.16.3 (MIT). Run every command below from
> `.claude/skills/insane-search/` (repo root relative), e.g. `cd .claude/skills/insane-search && python -m engine "<URL>" --trace`.
> Use `python` or `python3`, whichever this machine has. Observation logs go to `observations/` here and are gitignored.
...
```bash
cd .claude/skills/insane-search
...
```bash
mkdir -p ~/.insane-search/node && cp .claude/skills/insane-search/engine/templates/package.json ~/.insane-search/node/
